module.exports.config = {
  ...
  version: "1.0.0",
  credits: "Mirai Team & Mod by MrTomXxX", // Added canvas
...
module.exports.run = async function({ api, event, Users, Threads }) {
  ...
  const MTX = (event.author == leftParticipantFbId) ? "left by itself" : "has been kicked by the administrator";
  ...
  const MrT0mX = name.length > 10 ? name.slice(0, 10) + "..." : name;

  ctx.drawImage(background, 0, 0, canvas.width, canvas.height);
  ctx.drawImage(roundAvatarImg, canvas.width / 2 - 500, canvas.height / 2 - 200, 420, 420);
  ctx.font = '100px CustomFont';
  ctx.fillStyle = '#FFF';
  ctx.fillText(MrT0mX, canvas.width / 2 - 60, canvas.height / 2 + 90);

  ctx.font = '40px CustomFont';
  ctx.fillText(MTX, canvas.width / 2 - 50, canvas.height / 2 + 140);
