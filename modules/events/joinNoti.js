module.exports.config = {
  ...
  version: "1.0.0",
  credits: "Mirai-Team", // FIXED BY MrTomXxX
...
const fontlink =
  "https://drive.google.com/u/0/uc?id=1ZwFqYB-x6S9MjPfYm3t3SP1joohGl4iw&export=download";
  ...
  if (
    ...
    axios
      ...
      .then((response) => {
        ...
        return api.sendMessage("Hey There!", event.threadID, () =>
          api.sendMessage(
            {
    ...
    try {
      ...
      for (id in event.logMessageData.addedParticipants) {
        ...
        iduser.push(
          event.logMessageData.addedParticipants[id].userFbId.toString()
        );
      ...
      for (let o = 0; o < event.logMessageData.addedParticipants.length; o++) {
        ...
        ctx.font = `155px Semi`;
        ctx.fillText(
          `${event.logMessageData.addedParticipants[o].fullName}`,
          canvas.width / 2 + 20,
          canvas.height / 2 + 100
        );
        ctx.save();
        ...
        ctx.fillText(
          `You are the ${number}${suffix} member of this group`,
          canvas.width / 2 - 15,
          canvas.height / 2 + 350
        );
        ctx.restore();
