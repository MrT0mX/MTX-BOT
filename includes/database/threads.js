module.exports = function ({ api }) {
  ...
  async function setData(threadID, options, callback) {
    try {
      ...
      if (!threadsData.hasOwnProperty(threadID))
        throw new Error(
          `Threads with ID: ${threadID} does not exist in Database`
        );
  ...
  async function delData(threadID, callback) {
    try {
      ...
      await saveData(threadsData);
      if (callback && typeof callback == "function")
        callback(null, "REMOVE THREAD" + threadID + "SUCCESS");
      return true;
    } catch (error) {
      if (callback && typeof callback == "function") callback(error, null);
      return false;
      ...
      for (singleData of threadInfo.userInfo) {
        if (singleData.gender != undefined) {
          try {
            ...
            logger.log(
              global.getText("handleCreateDatabase", "newUser", singleData.id),
              "DATABASE"
            );
          } catch (e) {
            console.log(e);
