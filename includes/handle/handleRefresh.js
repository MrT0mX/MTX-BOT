    ...
    try {
      ...
      switch (logMessageType) {
        case "log:thread-admins": {
          ...
          logger.log(
            "Refresh the list of admins in the group " + threadID,
            "UPDATE DATA"
          );
        ...
        case "log:subscribe": {
          ...
          var data = await Threads.setData(event.threadID, {
        ...
        case "log:unsubscribe": {
          if (logMessageData.leftParticipantFbId == api.getCurrentUserID()) {
            logger.log(
              "Perform group data deletion " + threadID,
              "DELETE DATA"
            );
            const index = global.data.allThreadID.findIndex(
              (item) => item == threadID
            );
            ...
            const index = dataThread.participantIDs.findIndex(
              (item) => item == logMessageData.leftParticipantFbId
            );
            dataThread.participantIDs.splice(index, 1);
            if (
              dataThread.adminIDs.find(
                (i) => i.id == logMessageData.leftParticipantFbId
              )
            ) {
              dataThread.adminIDs = dataThread.adminIDs.filter(
            ...
            logger.log(
              "Perform user deletion " + logMessageData.leftParticipantFbId,
              "DELETE DATA"
            );
            await setData(threadID, { threadInfo: dataThread });
          }
